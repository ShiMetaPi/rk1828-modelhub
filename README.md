...
三个 demo 套路一样，就三步：**进目录 → 拉模型 → 启动**。

```bash
cd <demo 目录>                                         # chat / vl / tts
...
浏览器打开对应端口：聊天 **8089** · 视频问答 **8080** · 语音合成 **8088**。具体步骤、模型大小、板上路径都在各自的 README 里，点进去照着做就行。
...
板子的软件要求：聊天 demo 要系统自带 `rkllm3-server`；视频问答和语音合成要在板上现编译一次 C++ 引擎（`g++` + RKNN3 运行库）。启动脚本会自动探测这些依赖，缺什么直接告诉你。
